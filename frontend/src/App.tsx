...
import { generateAndExecuteGraph } from './services/exploratoryService';
...
function App() {
  ...
  // Exploratory prompt state
  const [exploratoryLoading, setExploratoryLoading] = useState(false);
  const [exploratoryError, setExploratoryError] = useState<string | null>(null);
  const [exploratoryChartData, setExploratoryChartData] = useState<GeneratedChartData | null>(null);
  ...
  const handleExploratoryPrompt = async (prompt: string) => {
    setExploratoryLoading(true);
    setExploratoryError(null);
    setShowVisualizationPopup(true);

    try {
      const result = await generateAndExecuteGraph(prompt);
      setExploratoryChartData(result);
    } catch (error) {
      console.error('Error processing exploratory prompt:', error);
      setExploratoryError(error instanceof Error ? error.message : 'Failed to generate visualization');
    } finally {
  ...
  return (
    <div className="h-screen w-screen bg-posthog-bg-primary flex flex-col overflow-hidden">
          ...
          <div className="w-3/5 p-4 overflow-hidden">
            <div className="h-full flex flex-col space-y-4">
              ...
              <div className="mb-3">
                ...
                {/* Exploratory Prompt - Compact */}
                <ExploratoryPromptBox
                  onSubmit={handleExploratoryPrompt}
                  isLoading={exploratoryLoading}
                />
              </div>
      ...
      {/* Visualization Popup for Exploratory Prompts */}
      <VisualizationPopup
        isOpen={showVisualizationPopup}
        onClose={closeVisualizationPopup}
        chartData={exploratoryChartData}
        isLoading={exploratoryLoading}
        error={exploratoryError}
      />
